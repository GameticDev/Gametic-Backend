...
.lean();
 ...
 // Transform the data to ensure bookedSlot is always populated
  const turfsWithBookedSlots = allTurf.map(turf => {
    // Use the virtual property if available (computedBookedSlots)
    const bookedSlots = (turf as any).computedBookedSlots || 
      // Fallback to manual calculation if virtuals didn't work
      turf.bookings?.reduce((acc: any[], booking: any) => {
        const dateStr = new Date(booking.date).toISOString().split('T')[0];
        const existing = acc.find(item => item.date === dateStr);
        
        if (existing) {
          existing.slots.push({ start: booking.startTime, end: booking.endTime });
        } else {
          acc.push({
            date: dateStr,
